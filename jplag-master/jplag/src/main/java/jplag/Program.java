...
public class Program implements ProgramI {
	...
	private void compare() throws jplag.ExitException {
		...
		for (i = 0; i < (size - 1); i++) {
			...
			for (j = (i + 1); j < size; j++) {
				...
				MydatabaseMethod mydatabaseMethod =new MydatabaseMethod();//qiubochen change
				mydatabaseMethod.insert(s1.name,s2.name,match.percent());//qiubochen change
				// histogram:
	 ...
	 */
	private void parseAll() throws jplag.ExitException {//qiubochen printf x submissions parsed successfuly
		if (submissions == null) {
	...
	private void writeResults(int[] dist, SortedVector<AllMatches> avgmatches, SortedVector<AllMatches> maxmatches,
		...
		//this.report.write(f, dist, avgmatches, maxmatches, minmatches, clustering, options);//qiubochen change this can write htmlfile
